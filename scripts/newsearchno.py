...
def make_search(**kwargs):
    ...
    link_in_Linkedin = []
    people_togheter=""
    if str(kwargs_login) =="1":
        print("Trying after login")
        for page in range(1, n_pages):
            try:
                url = URL + "&start="+str((page - 1) * 10)+ "&hl=en"
                kwargs_driver.get(url)        
                soup = BeautifulSoup(kwargs_driver.page_source, 'html.parser')
                search_links = soup.find_all('div', class_="yuRUbf")
    ...
    for page in range(1, n_pages):
        ...
        kwargs_driver.get(url)        
        try:
            name=kwargs_driver.find_element(By.XPATH,"/html/body/div[6]/div/div[13]/div[1]/div[2]/div[2]/div/div/div[1]/div/block-component/div/div[1]/div[1]/div/div/div[1]/div/div/div[1]/div/div[1]/div[2]/div/div[1]/a")
            name= name.text
            ...
            try:
                name=kwargs_driver.find_element(By.CLASS_NAME,"IZ6rdc")
                name= name.text
    ...
    LINK_LINKEDIN = ""    
    if have_the_name:
        response += f"\n{get_resume(name, kwargs_driver)}"
        #you need to get the biblio here
        ########Biblio=generate_google_search_url(f"{name}+{args[0]}+wikipedia")
        #LinkedIn_link=ImFeelLucky(f"{name}+{kwargs_company}+{kwargs_position}")        
        ...
        if kwargs_company and kwargs_position:
            LinkedIn_link=ImFeelLucky(f"{name}+{kwargs['company']}+{kwargs['position']}")  
    ...
    # I will try get the name
        ...
        if len(linkedin_links_list) > 0:
            ...
            if not name and LinkedIn_link:
                ...
                response += f"Name: {name}"
                response += f"\n{get_resume(name, kwargs_driver)}"
                LINK_LINKEDIN=ImFeelLucky(f"{name}+{kwargs_company}+{kwargs_position}")                
                print(LINK_LINKEDIN) 
                response += f"\nLink: {LINK_LINKEDIN}"
